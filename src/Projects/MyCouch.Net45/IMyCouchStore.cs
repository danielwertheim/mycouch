...
namespace MyCouch
{
    ...
    public interface IMyCouchStore : IDisposable
    {
        ...
        IObservable<Row> Stream(Query query);
        IObservable<Row<TValue>> Stream<TValue>(Query query);
        IObservable<Row<TValue, TIncludedDoc>> Stream<TValue, TIncludedDoc>(Query query);
    }
}